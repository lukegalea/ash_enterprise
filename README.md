...
|---|---|---|---|
| 1 | How do people prove who they are? | Password, magic link, OAuth2/OIDC and API keys via `ash_authentication` — an integration seam for the identity provider your enterprise already runs, not a second one. Federate to Okta, Entra ID or Keycloak and the provider owns MFA, passkeys and session policy; the in-app stack is the lightweight version the reference app needs to run standalone. SAML and WebAuthn in-app are the planned seam, for deployments that must authenticate in-app. | 🟡 Partial |
...
| 29 | How do enterprise customers bring their own identity provider? | Bringing your own IdP is the supported topology, not the exception. AshAuthentication federates to Okta, Entra ID, Keycloak or any OIDC provider through its OAuth2 strategy, and the provider — not this platform — owns MFA, password policy and directory truth. SAML is the one protocol that needs a first-party strategy; it is planned on the same seam for deployments that must authenticate in-app. | ⚪ Open |
...
| 30 | How do accounts appear and disappear when someone joins or leaves? | Joining and leaving is your directory's decision, and the IdP your enterprise runs is the system of record for it. Today users are created by registration and deactivated by a lifecycle transition; the planned SCIM seam is a resource-backed endpoint over those same actions, so the directory drives the lifecycle this platform already enforces — no parallel user store, no second copy of the truth. | ⚪ Open |
| 31 | What second factor is required, and how are sessions bounded? | With enterprise SSO the second factor is the provider's: Okta, Entra ID and Keycloak enforce MFA, step-up and session lifetime at the boundary this platform federates to. For the lightweight in-app login the reference app ships, MFA enforcement, step-up and configurable session lifetime are not built yet; they are planned as part of the identity seam, alongside WebAuthn. | ⚪ Open |
...
| 1 | Business rules and decision tables | ash_decisions — DMN, engine adopted not written | 🟡 Partial | [ADR 0028](docs/adr/0028-decisions-are-dmn.md) |
...
| 1 | Legacy migration, process modelling and decisions as platform extensions | ash_strangler + ash_bpmn + ash_decisions, first-party | ✅ Shipped | [ADR 0009](docs/adr/0009-strangler-and-bpmn-are-first-party.md) |
| 1 | Lineage and provenance | OpenLineage + Marquez | 🔵 Planned | [ADR 0012](docs/adr/0012-openlineage-and-marquez.md) |
| 1 | Rules and compliance as data | ash_rules (rule IR + swappable evaluators) and ash_compliance (governed layering compiled to immutable bundles, findings projected from events) | ✅ Shipped | [ADR 0035](docs/adr/0035-compliance-is-projected-from-events.md) |
