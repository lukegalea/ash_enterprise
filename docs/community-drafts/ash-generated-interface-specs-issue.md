# Upstream issue draft: generated type specs for code-interface actions (assign zachdaniel)
...
```sh
gh issue create --repo ash-project/ash --assignee zachdaniel \
  --title "feat: generate type specs for code interface actions" \
  --body-file docs/community-drafts/ash-generated-interface-specs-issue-body.md
```

---

## Body (save as the --body-file)
...
## Notes for Luke (not part of the issue)

- The fork link is public (fork of a public repo). Spot-check the branch name
  renders before filing.
- #2955 reference is deliberate — it connects this issue to the PR of ours
  he's already seen, which is the "method behind the patches" effect you
  wanted without saying it.
- If you'd rather start softer than an issue: ash Discussions has a
  Proposals category and the same body works there with @zachdaniel instead
  of assignment — but the issue is the better artifact since the feature is
  concrete and prototyped.
- Timing: file this after (or with) the personal note — the note's
  "look at lib/ash_agent_tools.ex" and this issue's evidence reinforce each
  other either order.
