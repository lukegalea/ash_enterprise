...
- **The mapping DSL's design** — why mappings are typed, tiered and invertible rather than strings — is recorded in
...
as a clearly-scoped ~200-line module if not. Either way, **notifications are not a pillar of this package**;
...
Two Spark extensions, not one: `AshStrangler.Resource` (the mapping) and `AshStrangler.Group` (the migration group —
what several resources over one legacy table share, plus the listener registration and the reconciliation schedule; see
§5.5). Splitting them means a resource file never mentions runtime concerns.

## 5. The DSL

A mapping is a **typed expression**, never a SQL string. That is the load-bearing constraint and everything in this
section follows from it: an expression can be classified, inverted, proven and printed by one printer, and a string can
only be concatenated and hoped over. The decision and its consequences are recorded in
[ADR 0008](../adr/0008-typed-invertible-legacy-mappings.md); the tier system and the inversion rules are documented at
length in `documentation/topics/the-transform-layer.md` in the `ash_strangler` repository.

### 5.1 The whole thing, for one resource

Two modules, because the legacy schema is a schema and Ash already has a way to describe one. The
**twin** is generated by introspecting the database — it is never hand-written, which is the point:

```elixir
# generated by `mix ash_strangler.gen.twin --relation legacy.users`
defmodule MyApp.Legacy.Users do
  use Ash.Resource, domain: MyApp.Legacy, data_layer: AshPostgres.DataLayer
  ...
  attributes do
    ...
    attribute :deleted_at,       :naive_datetime    # `timestamp`, no zone
  end

  identities do
    # Read from pg_index, not typed by hand. This is what lets AshPostgres map a
    # constraint violation back to the right Ash identity (§10.4).
...
defmodule MyApp.Accounts.User do
  ...
  attributes do
    ...
    # `one_of` is not decoration: `decode` checks exhaustiveness against it, and
    # `PutTotal` uses it to refuse an attribute value with no legacy encoding. An
    # unconstrained `:integer` here would leave `state_code: 7` writable and
    # unreadable, and a compile error is the only place that can be caught.
    attribute :state_code,  :atom,      allow_nil?: false, default: :active,
      constraints: [one_of: [:active, :passive, :pending, :suspended, :deleted]]
  ...
  postgres do
    ...
    # `VerifyNotMigrated` requires this in the view-backed phases: `table` names a
    # VIEW, and ordinary codegen would emit a `create table` for the same name,
    # producing a migration that cannot run (§6.1).
    migrate? false
  end

  strangler do
    # The one control knob. Everything below is derived; this decides which
    # artifacts are derived from it. It can live on a migration group instead, so
    # resources over one legacy table cannot disagree about it -- see §5.5.
    ...
    source MyApp.Legacy.Users do
      ...
      # A rename. Provably a bijection, so it needs no annotation -- and, being a
      # simple column reference, no trigger either (§10.2).
      map :email, from: :email

      # The zone IS the combinator: one declaration, and the inverse
      # (`attr AT TIME ZONE 'UTC'`) is derived from it rather than hand-written in
      # each generator. `VerifyTimestampZones` refuses a naive-to-timestamptz
      # mapping that does not name a zone, because a bare cast reads the session's
      # `TimeZone` GUC and is therefore wrong by a per-connection offset (§10.12).
      # It is also IMMUTABLE and therefore indexable; the bare cast is neither.
      map :archived_at, from: :deleted_at, zone: "UTC"

      # A declared bijection. Both directions come from this one map, and the
      # compiler checks injectivity, exhaustiveness against the attribute's
      # `one_of`, and the round trip. Two hand-written CASE expressions could not
      # be checked against each other at all, which is how a lifecycle map that
      # collapses `passive`, `pending` and `deleted` into `suspended` on write
      # reaches production looking correct.
      ...
      # Computed forward, provably not invertible. `because:` is required here
      # because it is quoted verbatim in the runtime error a developer reads at
      # 3am, and *forbidden* on a mapping the compiler proved invertible -- so the
      # prose cannot drift from the fact it claims to explain.
      # `||` is Ash's null-coalescing operator (it renders as SQL `coalesce`);
      # SQL's `||` concat is Ash's `<>`. Ash has no `coalesce/2` function at all.
      map :full_name,
        from: expr((first_name || "") <> " " <> (last_name || "")),
        writable?: false,
        because: "Not decomposable: 'de la Cruz' splits wrong, and there is no rule that fixes it."

      # Two legacy columns, one modern attribute. Forward only -- and here the
      # refusal is a policy decision rather than a limit of the grammar, which is
      # why it is an explicit opt-out.
      map :hashed_password,
      ...
      # Legacy indexes are not declared here. The twin's `identity` carries them,
      # read from pg_index, and `mix ash_strangler.check` verifies that against
      # the live catalogue -- rather than trusting a name typed twice.

      # How writes reach the base table, derived PER COLUMN rather than per
      # resource, choosing the weakest mechanism that works: a plain reference
      # needs nothing, an immutable derivation can be a GENERATED column, a
      # row-local one can be a BEFORE trigger on the base table, and only a
      # multi-table or non-invertible write needs INSTEAD OF. Declaring it is an
      # override with a real cost either way (§10.2).
      writes :auto
    end
  end
end
...
Note what does not appear anywhere in that file: the cast (`:citext` follows from `:string` →
`:ci_string`), the timestamp inverse, the legacy index, the source column list, and — with a migration
group — the phase and the namespace. Each would be a restatement of something Ash or Postgres already
knows, and a restatement is a place two descriptions can disagree.
...
SELECT
  ...
  CASE u.state WHEN 'active'    THEN 'active'
               WHEN 'passive'   THEN 'passive'
               WHEN 'pending'   THEN 'pending'
               WHEN 'suspended' THEN 'suspended'
               WHEN 'deleted'   THEN 'deleted'  END        AS state_code,
  (u.deleted_at AT TIME ZONE 'UTC')                        AS archived_at,
...
|---|---|
| a simple column reference or rename | **nothing** — Postgres auto-updates that column |
| an immutable, single-row derivation | a `GENERATED ALWAYS AS (…) STORED` column on the base table |
| a row-local derivation that must be written back | a `BEFORE` trigger on the **base table** plus a shadow column (pgroll's design); the view stays auto-updatable |
| a multi-table write, or a non-invertible mapping | `INSTEAD OF` triggers |
| `usage_counter? true` | all three, because the counter has nowhere else to live (§10.3) |

**The choice is per column, not per resource**, and that is a fact about Postgres rather than a stylistic preference.
`CREATE VIEW`'s updatability rule is column-level: a column is updatable if it is a simple reference to a
base-relation column, and *"otherwise the column is read-only, and an error will be raised if an `INSERT`, `UPDATE`, or
`MERGE` statement attempts to assign a value to it."* So a view may hold a **mix** of updatable and read-only columns,
and a computed column costs nothing until someone assigns to it. Measured on PostgreSQL 17.10, a single-table view with
two plain references and two computed columns kept `UPDATE` of the plain columns, `DELETE`, and
`INSERT … ON CONFLICT … RETURNING` with **no triggers at all** (§10.2). Deriving per resource would therefore pay the
trigger's whole price — upserts, correct `RETURNING`, `WITH CHECK OPTION` — for one computed column nobody writes to.
...
BEGIN
  ...
  VALUES (NEW.email,
          CASE NEW.state_code WHEN 'active' THEN 'active' WHEN 'passive' THEN 'passive'
                              WHEN 'pending' THEN 'pending' WHEN 'suspended' THEN 'suspended'
                              WHEN 'deleted' THEN 'deleted' END,
          NEW.archived_at AT TIME ZONE 'UTC')
  RETURNING * INTO rec;
...
### 5.3 Directionality is derived, and proven

The one thing this DSL will not do is guess, and the second thing it will not do is take a declaration on
trust. A hand-declared backward direction is a claim nothing checks — which is exactly how a mapping that
rewrites three of five lifecycle states on write passes review looking correct. So the backward direction
is **constructed by the tool**, and every mapping lands in one of four classifications:

| Form | Classification | Compile-time rule |
|---|---|---|
| `map :email, from: :email` | `IDENTITY` | a bijection by construction; needs no trigger at all |
| a Tier 1 combinator — `cast:`, `zone:`, `decode`, `affine`, `compose` | `TRANSFORMATION, invertible: :yes` | the inverse is derived, then **proven** by round-tripping over the legacy value space |
| a Tier 2 combinator — `coalesce`, `drop`, `concat`, `collapse` | `TRANSFORMATION, invertible: :semi` | needs exactly one declared datum (a default, a separator, a `set:`); the loss is recorded, not hidden |
| `writable?: false, because: …`, or any `fragment` | `MASKED` / opaque | the attribute is rejected on write with an error quoting `because:` |

Three rules hold that classification together:

- **`because:` is forbidden** on a mapping the compiler proved invertible, and required on an opt-out.
  Prose that is allowed to sit beside a fact the tool already knows is prose that will eventually
  contradict it; prose attached only where the tool knows nothing cannot. On the opt-out it is mandatory
  because it is quoted verbatim in the runtime error a developer reads at 3am.
- **The obligation to supply a backward direction is computed, not blanket.** The two tools closest to
  this problem both compute it: pgroll's validator requires both directions for `change_type`, only `up`
  for `set_not_null`, neither for `set_default`, and `add_column` has no `down` field at all; Reshape
  derives it from the schema — *"the `down` setting must be provided when the removed column is
  `NOT NULL` or doesn't have a default value"*. Demanding one everywhere trains authors to write
  something plausible to get past the compiler, which is worse than not asking.
- **The highest-value constraint is `PutGet`**, not "declare the inverse or say why": forward ∘ backward
  must be the identity **over the legacy value space**, because that is the only space containing rows
  the tool did not create. A tool that silently drops a column on write is the failure this package
  exists to prevent; a tool that silently *rewrites* one is worse, because the row still looks populated.

### 5.4 The primary key problem, in the DSL

Legacy integer keys against modern uuids is the first thing every user hits. Two strategies, declared:

```elixir
key :id, from: :id,       strategy: {:uuid_v5, namespace: "..."}  # derived; no legacy writes
key :id, from: :row_uuid, strategy: :identity                    # a real column, after the expand step
```

`Sql.View` raises `unsupported_strategy!` for anything else, rather than emitting SQL for a strategy it
cannot generate an index for.

`{:uuid_v5, …}` uses `uuid_generate_v5` from `uuid-ossp`. Verified against PostgreSQL 17.10:
`pg_proc.provolatile = 'i'`, genuinely `IMMUTABLE` (unlike `uuid_generate_v4` and `gen_random_uuid`,
both `'v'`), the expression index is created, and `EXPLAIN` confirms it is *used* through the view:
...
The extension emits that index automatically, because forgetting it turns every `Ash.get` into a
sequential scan. It only keeps working while the index expression and the query expression are
byte-identical — which is one of the reasons every generator prints through a single printer rather than
each assembling its own SQL, since two printers that agree today will not agree after one of them is
edited.

Two related volatility findings, since this is where they bite. A **hand-rolled** v5 over `pgcrypto`
defaults to `VOLATILE` and cannot be indexed even though it is genuinely immutable; report
`pg_proc.provolatile` and name the function rather than surfacing a bare Postgres error, and **never**
resolve it with `ALTER FUNCTION … IMMUTABLE`, which produces an index that disagrees with the heap and
returns wrong rows with no error. And `(naive AT TIME ZONE 'UTC')` *is* indexable — it resolves to
`timezone(text, timestamp without time zone)`, which is `IMMUTABLE` — while `(naive::timestamptz)` is
refused outright. So the form `VerifyTimestampZones` demands is the only one that can carry an index,
and the cast it rejects could never have carried one.
...
### 5.5 The migration group: what is shared, declared once

A migration group exists because a per-resource `phase` cannot express a constraint that is not
per-resource. Resources over one legacy table **must** move phase together; declared one at a time,
`Customer`, `Organization` and `Address` over one `legacy.accounts` can be left in three different phases
and each compiles fine, which is a half-migrated table nothing reports. Alongside that, the relation, the
key strategy, the namespace and every join get copy-pasted across all three — three times in the
package's own `test/support/demo.ex`, where the duplication is small enough to see and still happened.

So the group owns what is shared, and the resources inherit it:

```elixir
defmodule MyApp.Legacy.AccountsMigration do
  use AshStrangler.Group

  strangler do
    phase :read_from_legacy
    source MyApp.Legacy.Users
    namespace "6b1e8b2c-6f6d-4a4a-9f1a-5b0e0d3c4a71"

    repo MyApp.Repo
    listen? true              # starts the Postgrex.Notifications listener
...
That is what gives `VerifyPhaseTransition` a cross-resource agreement check to make at all — a verifier
looking at one resource cannot see the other two. `Spark.Dsl.Fragment` — which already exists for exactly
this — carries a shared `source` block into each resource without retyping it.
...
- **`VerifyIdentitiesBacked`** — an `identity` on a view is not enforced by anything, and worse, AshPostgres's
  ...
  identity must either be backed by a unique `identity` on the twin — which is read from `pg_index`, and from which the
  extension derives both `identity_index_names` and `skip_unique_indexes` — or be marked `enforced? false`. Requiring a
  real catalogue entry rather than a declaration is the point: a hand-typed index name is a claim about the database
  that nothing checks. Silently unenforced uniqueness on
...
`WITH CHECK OPTION` deserves the sharper statement: the same violating `INSERT` that a plain auto-updatable view
**refuses** succeeds under a trigger and the row lands in the base table. An enforced constraint does not become
absent, it becomes a no-op that writes.
...
**And the price is paid per column, not per view.** `CREATE VIEW`'s updatability rule is column-level: a column is
updatable if it is a simple reference to a base-relation column, *"otherwise the column is read-only, and an error will
be raised if an `INSERT`, `UPDATE`, or `MERGE` statement attempts to assign a value to it."* So a view may hold a
**mix** of updatable and read-only columns, and a computed column does not force a trigger — it forces an error only if
someone assigns to it. Measured on PostgreSQL 17.10, a single-table view with two plain references and two computed
columns, **no triggers**:
...
UPDATE mixed_v SET full_name = 'Nope';            -- ERROR: cannot update column "full_name"
DELETE FROM mixed_v WHERE id = 1;                 -- succeeds
INSERT … ON CONFLICT (email) DO UPDATE … RETURNING …  -- works, insert and conflict alike
```

**Upserts are lost to the trigger, not to the computation.**

**The design consequence is that triggers must be derived, not policy.** The tempting rule — "generate all three
`INSTEAD OF` triggers in `:dual_write` so nothing takes an ungoverned path" — would destroy upsert support on every
resource, including the authentication resources that need it most, and would do it to pay for computed columns nobody
writes to. The correct rule is:

> Generate `INSTEAD OF` triggers **only for the operations the mapping actually requires them for**, and make the
> requirement a derived property of the DSL, per column, choosing the weakest mechanism that works: a plain reference
> needs nothing; an immutable single-row derivation can be a `GENERATED … STORED` column; a row-local derivation that
> must be written back can be a `BEFORE` trigger on the *base table* plus a shadow column (pgroll writes no `INSTEAD
> OF` triggers at all on this basis); only a multi-table write, a genuinely non-invertible mapping, or a usage counter
> forces `INSTEAD OF`.

Do not try to detect any of this with `information_schema.views.is_updatable`. It is defined to pass
`include_triggers = false`, so on a two-table join view made writable purely by triggers it reports `NO` while the
`UPDATE` succeeds. Introspect with `pg_relation_is_updatable(oid, true)` and `pg_column_is_updatable/3`.

`VerifyNoUpserts` is conditional for exactly this reason: it rejects `upsert?: true` only on resources whose mapping forced a
...
The obvious SQL for a `timestamp` → `timestamptz` mapping is a plain cast, `(deleted_at)::timestamptz`.
When the legacy column is `timestamp` *without* time zone, which is what Rails,
...
**This is why the DSL asks, and refuses to compile if it is not told.** The deterministic SQL is
`(deleted_at AT TIME ZONE 'UTC')` — but only if the legacy column really is UTC. The generator cannot know that: it is
a fact about the legacy application, not about the schema, and guessing wrong moves every timestamp in the system by a
fixed offset. Three ways to handle that, in preference order:

1. **Make the DSL ask.** `map :archived_at, from: :deleted_at, zone: "UTC"` generates `AT TIME ZONE`, and
   `VerifyTimestampZones` rejects a naive-to-`timestamptz` mapping that does not say which zone. That is the
   `because:`-style move this package makes elsewhere: force the ambiguity to be resolved in the mapping, where it is
   reviewable, rather than inherited from whatever connection happens to run the query.
...
Option 1 is where this lands. Pinned by a regression test (`AshStrangler.RoundTripTest`, tagged `:hazard`) that asserts
the drift the bare cast produces *exists*, so the day Postgres changes that behaviour, something fails rather than
quietly starting to agree.
